...
#
# Adapted from graphdeco-inria/gaussian-splatting (scene/gaussian_model.py). Deviations
# from the original, both needed because the compute backend (Colab GPU vs Colab CPU) was
# left undecided (see CLAUDE.md section 3):
#   1. Every hard-coded `device="cuda"` / `.cuda()` became `device=self.get_xyz.device` (or
#      an explicit `device` argument), so the class works unchanged on CPU.
#   2. `create_from_pcd`'s neighbor-distance initialization used `simple_knn._C.distCUDA2`
#      (a CUDA extension) upstream. Replaced with a pure CPU/NumPy k-NN (scipy.spatial.cKDTree)
#      computing the same quantity (mean squared distance to the 3 nearest neighbors) — this
#      only runs once at startup, not in the training loop, so the CPU cost is negligible
#      even for a scene's full sparse point cloud.
# Everything else (parameter layout, densify/prune/clone/split logic, save/load format) is
# unmodified so `.ply` checkpoints stay compatible with the standard 3DGS viewers/tools.
...
def _mean_sq_dist_to_nearest_neighbors(points: np.ndarray, k: int = 3) -> np.ndarray:
    """CPU replacement for simple_knn._C.distCUDA2: mean squared distance from each point
    to its k nearest neighbors (excluding itself)."""
    tree = cKDTree(points)
    dists, _ = tree.query(points, k=k + 1)  # includes the point itself at index 0
    dists = dists[:, 1:]  # drop self (distance 0)
    return np.mean(dists ** 2, axis=1)
...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float, device="cpu"):
        self.spatial_lr_scale = spatial_lr_scale
        points_np = np.asarray(pcd.points)
        fused_point_cloud = torch.tensor(points_np).float().to(device)
        ...
        dist2 = np.clip(_mean_sq_dist_to_nearest_neighbors(points_np, k=3), 1e-7, None)
        dist2 = torch.tensor(dist2, dtype=torch.float, device=device)
    ...
    def load_ply(self, path, device="cpu"):
